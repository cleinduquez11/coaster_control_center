...
class VisSandBoxVisualization extends StatefulWidget {
  // final List<String> overlayImages;
  final int length;
  final String url;

  const VisSandBoxVisualization(
      {super.key, required this.length, required this.url});
  @override
  State<VisSandBoxVisualization> createState() =>
      VisSandBoxVisualizationState();
}

class VisSandBoxVisualizationState extends State<VisSandBoxVisualization> {
  late Timer _timer;
  int _currentIndex = 0;
  ...
  void initState() {
    ...
    oI = List.generate(
        widget.length, (index) => '${widget.url}${index + 1}.png');
    // print(oI);
  ...
  void _startSlideshow() {
    _timer = Timer.periodic(const Duration(milliseconds: 100), (timer) {
      setState(() {
        _currentIndex = (_currentIndex + 1) % widget.length;
      });
              ...
              OverlayImageLayer(
                overlayImages: [
                  OverlayImage(
                    ...
                    opacity: 0.75,
                    // imageProvider:
                    imageProvider: NetworkImage(oI[_currentIndex]),
